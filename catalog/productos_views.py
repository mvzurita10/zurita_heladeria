...
from .mongo_serializers import ProductServiceSerializer

col = db["product_services"]
...
def product_services_list_create(request):
...
@permission_classes([IsAuthenticated])
def product_services_detail(request, id: str):
    _id = oid_or_none(id)
    if _id is None:
        return Response({"detail": "id invalido"}, status=status.HTTP_400_BAD_REQUEST)
    ...
    if request.method in ["PUT", "PATCH"]:
        serializer = ProductServiceSerializer(data=request.data, partial=(request.method == "PATCH"))
        serializer.is_valid(raise_exception=True)
